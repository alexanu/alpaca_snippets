...
# Usage: "$ python main.py --lot=2000 TSLA FB AAPL"
# minute level bar streaming from Polygon via Websockets
# handles multiple stocks concurrently

'''
buy signal: 20 minute MA crossover => immediately sell the position at or above the entry price. 
The assumption is that the market is bouncing upward when this signal occurs in a short period of time.

The buy signal is calculated as soon as a minute bar arrives, 
which typically happen about 4 seconds after the top of every minute 
(this is Polygon's behavior for minute bar streaming).
'''

import alpaca_trade_api as alpaca
import asyncio # handles multiple stocks concurrently
import pandas as pd
import pytz
import sys
import logging
...
class ScalpAlgo:
    ...
    def checkup(self, position):
        ...
        now = self._now()
        order = self._order
        # The buy order is canceled after 2 minutes if it does not fill
        if (order is not None and order.side == 'buy' 
            and now - order.submitted_at.tz_convert(tz='America/New_York') > pd.Timedelta('2 min')):
            last_price = self._api.get_last_trade(self._symbol).price
            self._l.info(f'canceling missed buy order {order.id} at {order.limit_price} '
                         f'(current price = {last_price})')
            self._cancel_order()

        #  liquidates all watching positions with market order at the end of market hours
    ...
    def _calc_buy_signal(self):
        ...
        # buy signal: 20 minute MA crossover
        if closes[-2] < mavg[-2] and closes[-1] > mavg[-1]:
            self._l.info(f'buy signal: closes[-2] {closes[-2]} < mavg[-2] {mavg[-2]} '
    ...
    def on_bar(self, bar):
        self._bars = self._bars.append(
            pd.DataFrame({'open': bar.open,'high': bar.high,'low': bar.low,'close': bar.close,'volume': bar.volume}, 
                        index=[pd.Timestamp(bar.timestamp, tz=pytz.UTC)]))

        self._l.info(f'received bar start: {pd.Timestamp(bar.timestamp)}, close: {bar.close}, len(bars): {len(self._bars)}')
