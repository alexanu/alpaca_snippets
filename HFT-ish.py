...
# Source: https://github.com/alpacahq/example-hftish
# Description:
    '''
    Capture slight moves in the bid/ask spread for high-volume stocks
    Details: https://alpaca.markets/learn/hft-like-trading-algorithm/
    Paper: https://www.palmislandtraders.com/econ136/hftois.pdf
    ...
    make many trades on the same security each day, so any account running it will quickly encounter Pattern Day Trader rule
    Please make sure your account balance is well above $25,000
...
def run(args):
    ...
    async def on_trade(conn, channel, data):
        ...
        if (data.timestamp <= (quote.time + pd.Timedelta(np.timedelta64(50, 'ms')))): 
            return # The trade came too close to the quote update
        if data.size >= 100: # large enough to follow ...
            if (data.price == quote.ask
                and quote.bid_size > (quote.ask_size * 1.8) # bid vs ask quantities (order book imbalance) indicate a movement in that direction
                and (position.total_shares + position.pending_buy_shares) < max_shares - 100): # not buying or selling more than we should 
                try:
                    o = api.submit_order(# buy at the ask
                        symbol=symbol, qty='100', side='buy',type='limit', time_in_force='day',limit_price=str(quote.ask))
                ...
                try:
                    o = api.submit_order( # sell at the bid
                        symbol=symbol, qty='100', side='sell',type='limit', time_in_force='day',limit_price=str(quote.bid))
                    # Approximate an IOC order by immediately cancelling
                    api.cancel_order(o.id)
    ...
    async def on_trade_updates(conn, channel, data):
        ...
        if event == 'fill':
            ...
            position.update_filled_amount(data.order['id'], int(data.order['filled_qty']),data.order['side'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--symbol', type=str, default='SNAP',help='Symbol you want to trade.')
    parser.add_argument('--quantity', type=int, default=500, help='Maximum number of shares to hold at once. Minimum 100.')
    parser.add_argument('--key-id', type=str, default=None,help='API key ID')
    parser.add_argument('--secret-key', type=str, default=None,help='API secret key')
    parser.add_argument('--base-url', type=str, default=None, help='set https://paper-api.alpaca.markets if paper trading')
